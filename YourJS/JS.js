...
(function(__global, __EMPTY_OBJECT, __EMPTY_ARRAY, undefined) {
  ...
  function typeOf(o, t) {
    o = o == undefined
      ? o === undefined
        ? 'undefined'
        : 'null'
      : __EMPTY_OBJECT.toString.call(o).slice(8, -1);
    return t ? t == o : o;
  }
  ...
  'Arguments Array Boolean Date Error Function null Number Object RegExp String undefined'.replace(/(\w)(\w+)/g, function(typeName, letter1, otherLetters) {
  ...
  function param(callback, arrFields) {
    for (var t, defaults = [], len = arrFields.length, i = len; i--;) {
      t = typeOf(t = arrFields[i], 'Array') ? t : [t];
      defaults[i] = t[1];
      arrFields[i] = t[0];
    }
    ...
    return function(fields) {
      ...
      for (var t, args = slice(arguments, 0), i = len; i--;) {
        args.unshift(fields && (has(fields, t=arrFields[i]) ? fields[t] : defaults[i]));
      }
  ...
  'cos sin tan'.replace(/\w+/g, function(name, i, R, fn) {
    R = Math.PI / 180;
    fn = YourJS[name] = function(angle, opt_usingDegrees) {
      if (opt_usingDegrees) {
        angle *= R;
      }
      return Math[name](angle);
    };
    YourJS[i ? i == 4 ? 'csc' : 'cot' : 'sec'] = function(angle, opt_usingDegrees) {
      return 1 / fn(angle, opt_usingDegrees);
    };
  });
  ...
  function join(arr, opt_delimiter, opt_beforeEach, opt_afterEach) {
    ...
    return arr.length
      ? opt_beforeEach + arr.join(
          opt_afterEach
          + (opt_delimiter || opt_delimiter == '' ? opt_delimiter : ',')
          + opt_beforeEach
        ) + opt_afterEach
      : '';
  }
  ...
  function isComposite(num) {
    return num > 1 ? !isPrime(num) : false;
  }
  ...
  (function(baseTypeOf, EXTRA_TYPES, l) {
    (typeOf = function(value, opt_typeName) {
      var i = 0, typeName = baseTypeOf(value);
      if (typeName == 'Object') {
        for (; i < l; i++) {
      ...
      return opt_typeName ? opt_typeName == typeName : typeName;
    });
